...
pub(crate) fn parse_static_path(
    ...
    validate_public_static_request(positioning, width, height, scale, overlays.is_empty())?;
...
fn validate_range(value: f64, name: &str, min: f64, max: f64) -> Result<(), IngressError> {
    if (min..=max).contains(&value) {
        Ok(())
    } else {
        Err(invalid(format!("{name} must be in [{min}, {max}]")))
    }
}

fn validate_lon(value: f64, name: &str) -> Result<(), IngressError> {
    validate_range(value, name, POSITION_MIN_LON, POSITION_MAX_LON)
}

fn validate_lat(value: f64, name: &str) -> Result<(), IngressError> {
    validate_range(value, name, POSITION_MIN_LAT, POSITION_MAX_LAT)
}

fn validate_zoom(value: f64) -> Result<(), IngressError> {
    validate_range(value, "zoom", POSITION_MIN_ZOOM, POSITION_MAX_ZOOM)
}

/// Bearings are periodic, so every finite degree value is meaningful and
/// MapLibre Native may normalize it. Non-finite values are not a valid camera
/// input and must not cross the native boundary.
fn validate_bearing(value: f32) -> Result<(), IngressError> {
    if value.is_finite() {
        Ok(())
    } else {
        Err(invalid("bearing must be finite"))
    }
}
...
/// Validate the aggregate invariants that are not established while public
/// static-image components are parsed. Overlay, camera, query, and addlayer
/// payloads have already passed their format-specific validators at this point.
fn validate_public_static_request(
    ...
    validate_auto_has_overlays(positioning, overlays_empty)
}

/// Enforce every render-safety invariant on a fully-built [`RenderRequest`]
/// received from a peer. Serde proves syntax, not safe dimensions, camera
/// values, overlay bounds, or nested addlayer invariants, so forwarded ingress
/// must retain this complete validation pass before reaching the renderer.
pub(crate) fn validate_render_request(
    ...
    match request {
        RenderRequest::StaticImage {
            ...
            validate_static_dimensions(*width, *height, scale)?;
            if overlays.len() > biei_core::types::MAX_STATIC_OVERLAYS {
                return Err(invalid(format!(
                    "request has more than {} overlays",
                    biei_core::types::MAX_STATIC_OVERLAYS
                )));
            }
            for overlay in overlays {
                crate::http::overlay::validate_static_overlay(overlay)
                    .map_err(|err| invalid(format!("invalid static overlay: {err}")))?;
            }
            validate_positioning(positioning, overlays.is_empty())?;
            crate::http::query::validate_before_layer(before_layer.as_deref())?;
            crate::http::query::validate_padding(*padding)?;
            if let Some(addlayer) = addlayer {
                crate::http::addlayer::validate_addlayer(addlayer)?;
            }
        }
        RenderRequest::Tile { z, x, y, tile_size } => {
            crate::http::tile::validate_tile_coordinate(*z, *x, *y)?;
            crate::http::tile::validate_tile_size(*tile_size)?;
        }
    }
    Ok(())
}

/// Validate the camera/positioning of a built request. `overlays_empty` gates
/// `auto`, which fits the camera to overlay geometry and is meaningless without
/// any.
fn validate_positioning(
    ...
    match positioning {
        Positioning::Auto => validate_auto_has_overlays(*positioning, overlays_empty)?,
        Positioning::Center {
            lon,
            lat,
            zoom,
...
fn validate_auto_has_overlays(
    positioning: Positioning,
    overlays_empty: bool,
) -> Result<(), IngressError> {
    if positioning == Positioning::Auto && overlays_empty {
        Err(invalid(
...
mod tests {
    use super::*;
    use crate::http::tile::TILE_SIZE;
    use biei_core::style_catalog::StyleDefinition;
    use biei_core::types::{ImageFormat, StyleId};
    ...
    fn parse_static_with_options(
        ...
        let style_id = crate::http::path::resolve_style_id(&parts[..static_index])?;
        parse_static_path(
            &parts,
            static_index,
            style_id,
    ...
    fn parse_static_threads_addlayer_into_render_request() {
        ...
        let addlayer = crate::http::addlayer::parse_addlayer_from_query(
            Some(&format!("addlayer={json}")),
    ...
    fn static_request(width: u16, height: u16, positioning: Positioning) -> RenderRequest {
        RenderRequest::StaticImage {
            positioning,
            width,
            height,
    ...
    fn center(lon: f64, lat: f64) -> Positioning {
        Positioning::Center {
            lon,
            lat,
            zoom: 10.0,
    ...
    #[test]
    fn public_static_aggregate_validation_retains_dimensions_and_auto_requirements() {
        assert!(
            validate_public_static_request(center(0.0, 0.0), 1921, 256, Scale::X1, true).is_err()
        );
        assert!(
            validate_public_static_request(Positioning::Auto, 256, 256, Scale::X1, true).is_err()
        );
        assert!(
            validate_public_static_request(Positioning::Auto, 256, 256, Scale::X1, false).is_ok()
        );
    }

    #[test]
    fn validate_render_request_accepts_public_shaped_requests() {
        assert!(
            validate_render_request(&static_request(256, 256, center(139.7, 35.6)), Scale::X2)
                .is_ok()
        );
        assert!(
    ...
    #[test]
    fn validate_render_request_rejects_forwarded_unsafe_values() {
        // Oversized static dimensions (would drive a huge RGBA allocation).
        assert!(
            validate_render_request(&static_request(20_000, 20_000, center(0.0, 0.0)), Scale::X2)
                .is_err()
        );
        // Non-finite / out-of-range camera (would crash MapLibre Native).
        assert!(
            validate_render_request(&static_request(256, 256, center(f64::NAN, 0.0)), Scale::X1)
                .is_err()
        );
        // `auto` with no overlays has nothing to fit.
        assert!(
            validate_render_request(&static_request(256, 256, Positioning::Auto), Scale::X1)
                .is_err()
        );
        // Out-of-range tile coordinate and oversized tile edge.
        assert!(
            validate_render_request(
                &RenderRequest::Tile {
                    z: 1,
    ...
    #[test]
    fn validate_render_request_rejects_nested_forwarded_values() {
        use biei_core::types::{GeoJsonOverlay, LngLat, PathOverlay, StaticOverlay};

        let mut path_request = static_request(256, 256, center(0.0, 0.0));
        let RenderRequest::StaticImage { overlays, .. } = &mut path_request else {
            unreachable!();
        };
        overlays.push(StaticOverlay::Path(PathOverlay {
            stroke_width: None,
            stroke_color: None,
            stroke_opacity: None,
            fill_color: None,
        ...
        assert!(validate_render_request(&path_request, Scale::X1).is_err());

        let mut geojson_request = static_request(256, 256, center(0.0, 0.0));
        let RenderRequest::StaticImage { overlays, .. } = &mut geojson_request else {
            unreachable!();
        };
        overlays.push(StaticOverlay::GeoJson(GeoJsonOverlay {
            feature_collection: serde_json::json!({
                "type": "Feature",
                "geometry": {
                    "type": "LineString",
                    "coordinates": vec![serde_json::json!([0.0, 0.0]); 5_001],
                },
            }),
        }));
        assert!(validate_render_request(&geojson_request, Scale::X1).is_err());

        let mut query_request = static_request(256, 256, center(0.0, 0.0));
        ...
        *before_layer = Some("invalid/layer".to_string());
        padding.top = crate::http::query::MAX_PADDING + 1;
        assert!(validate_render_request(&query_request, Scale::X1).is_err());

        let mut addlayer_request = static_request(256, 256, center(0.0, 0.0));
        let RenderRequest::StaticImage { addlayer, .. } = &mut addlayer_request else {
            unreachable!();
        };
        *addlayer = Some(AddLayer {
            json: "x".repeat(crate::http::addlayer::MAX_ADDLAYER_JSON_BYTES + 1),
            hash: 0,
            source: None,
        });
        assert!(validate_render_request(&addlayer_request, Scale::X1).is_err());
    }

    #[test]
    fn validate_render_request_bounds_overlay_count() {
        use biei_core::types::{LngLat, MAX_STATIC_OVERLAYS, PathOverlay, StaticOverlay};
        ...
        let overlay = || {
            StaticOverlay::Path(PathOverlay {
                ...
                fill_color: None,
                fill_opacity: None,
                coordinates: vec![LngLat { lon: 0.0, lat: 0.0 }],
            })
        };
        let with_overlays = |count: usize| RenderRequest::StaticImage {
            positioning: center(0.0, 0.0),
            width: 256,
            height: 256,
            overlays: std::iter::repeat_with(overlay).take(count).collect(),
            before_layer: None,
            padding: Padding::default(),
            addlayer: None,
        };

        // Exactly the cap is accepted; one over is rejected (public ingress
        // parity — a peer must not smuggle an unbounded overlay list).
        assert!(validate_render_request(&with_overlays(MAX_STATIC_OVERLAYS), Scale::X1).is_ok());
        assert!(
            validate_render_request(&with_overlays(MAX_STATIC_OVERLAYS + 1), Scale::X1).is_err()
        );
    }
}
