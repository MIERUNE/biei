...
Constraints that come from the MapLibre Native engine itself — thread affinity, build-time-fixed MapMode / pixel_ratio, no render cancellation, GeoJSON normalization, no provider-specific service features — live in `production-spec.md §8.6`. Keep those separate from things a binding could actually fix.
...
`production-spec.md §8.6`, not a Rust-binding wishlist item. And things biei
...
FileSource response fields across the C++ bridge. biei consumes the crates.io
