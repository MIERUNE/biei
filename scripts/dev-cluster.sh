...
for ((i = 0; i < NUM_NODES; i++)); do
  ...
  advertise_addr="${ADVERTISE_HOST}:${http_port}"
  ...
  echo "starting ${node_id}: http=http://${bind_addr} advertise=${advertise_addr} gossip=${gossip_bind} log=${LOG_FILE}"
...
biei dev cluster started.
  ...
  health:      http://${HTTP_HOST}:${BASE_HTTP_PORT}/_internal/healthz
  ready:       http://${HTTP_HOST}:${BASE_HTTP_PORT}/_internal/readyz
  metrics:     http://${HTTP_HOST}:${BASE_HTTP_PORT}/_internal/metrics
  log:         ${LOG_FILE}
