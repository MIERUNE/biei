...
use crate::types::{
    CachePolicy, CompletedInfo, DeadlineStage, ImageFormat, InternalTask, RejectionReason,
    RenderOutput, RenderRequest, RequestId, RouteTier, Scale, SourceHash, SourceRef, StyleId,
    StyleRevision, TaskId, TaskOutcome, TaskResult, WorkerId,
};
...
impl InternalTask {
    pub fn to_wire(&self, now: Instant) -> WireTask {
        let budget = self.deadline.saturating_duration_since(now);
        WireTask {
            id: self.id,
...
impl OutcomeHeader {
    pub fn from_task_outcome(
        ...
        let result = match result {
            ...
            TaskResult::Failed { error } => OutcomeResult::Failed { error },
        };
    ...
    pub fn into_task_outcome(
        ...
        let result = match result {
            ...
            OutcomeResult::Failed { error } => TaskResult::Failed { error },
        };
