...
fn body_permit_wait_and_inflight_metrics_are_registered() {
    ...
    assert!(
        names
            .iter()
            .any(|name| name == "biei_mln_resource_body_wait_seconds")
    );
    assert!(
        names
            .iter()
...
fn singleflight_does_not_mix_regular_and_background_refreshes() {
    let resource = ResourceRequestKey::test_key("https://resource.test/tile", ResourceKind::Tile);
...
async fn cancelled_flight_leader_wakes_waiters_and_removes_entry() {
    let key = FlightKey {
        resource: ResourceRequestKey::test_key("https://resource.test/tile", ResourceKind::Tile),
        persistent: true,
        priority: "regular",
        semantics: FlightRequestSemantics::default(),
    };
