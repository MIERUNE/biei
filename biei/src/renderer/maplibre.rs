...
use crate::renderer::maplibre_actor::{
    RenderTaskView, RendererActor, RendererActorConfig, ResolvedStyle,
};
...
impl Drop for InFlightStyleLoad<'_> {
    fn drop(&mut self) {
        ...
        self.inflight
            .lock()
            .expect("in-flight style load map poisoned")
            .remove(&self.key);
...
impl MapLibreRenderer {
    pub fn spawn(config: RendererActorConfig) -> Result<Self, RendererError> {
        Ok(Self {
            actor: RendererActor::spawn(config)?,
        })
    }
...
mod tests {
    use super::*;
    use crate::renderer::maplibre_actor::BlockingRenderBackend;
