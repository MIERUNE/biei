...
use crate::types::{RejectionReason, RequestId, TaskOutcome, TaskResult};
...
pub fn response_from_outcome(outcome: TaskOutcome) -> IngressResponse {
    ...
    match outcome.result {
        ...
        TaskResult::Failed { error } => response_from_failure(&error).with_request_id(&request_id),
    }
}
...
fn response_from_failure(error: &str) -> IngressResponse {
    if error.contains("render timeout") {
        IngressResponse::json(504, "render_timeout", error).with_retry_after("1")
    } else if error.contains("renderer actor dead") {
        IngressResponse::json(500, "renderer_dead", error).with_retry_after("1")
    } else if error.contains("style load failed") {
        IngressResponse::json(502, "style_unavailable", error)
    } else if error.contains("style not ready") {
        IngressResponse::json(500, "style_not_ready", error)
    } else if error.contains("source fetch failed") {
        IngressResponse::json(502, "source_unavailable", error)
    } else {
        IngressResponse::json(500, "render_failed", error)
    }
}
