...
struct Cli {
    ...
    cluster: bool,
    #[arg(long, env = "BIEI_BIND", default_value = DEFAULT_BIND)]
    bind: SocketAddr,
    #[arg(long, env = "BIEI_ADVERTISE_ADDR")]
...
impl Options {
    ...
    fn from_cli(cli: Cli) -> anyhow::Result<Self> {
        ...
        let gossip_seeds: Vec<_> = cli
            ...
            .collect();
        let advertise_addr = cli.advertise_addr.unwrap_or(cli.bind);
        if !cli.cluster && !gossip_seeds.is_empty() {
            bail!("use --cluster to enable cluster mode");
        }
        if cli.cluster && advertise_addr.ip().is_unspecified() {
            bail!("cluster mode needs a non-wildcard --advertise-addr");
        }
    ...
    pub fn cluster_config(&self) -> ClusterConfig {
        ClusterConfig {
            ...
            render_output_cache_capacity_bytes: DEFAULT_RENDER_OUTPUT_CACHE_BYTES,
        }
    }
}
...
mod tests {
    ...
    fn parses_cluster_advertise_addr() {
        ...
        assert!(opts.cluster);
        assert_eq!(opts.advertise_addr, "127.0.0.1:18080".parse().unwrap());
    }
    ...
    fn rejects_cluster_with_wildcard_advertise_addr() {
        ...
        assert!(format!("{err:#}").contains("non-wildcard --advertise-addr"));
    }
    ...
    fn accepts_cluster_seed_node_without_gossip_seeds() {
        ...
        assert_eq!(opts.advertise_addr, "127.0.0.1:8080".parse().unwrap());
    }
