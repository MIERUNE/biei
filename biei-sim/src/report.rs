...
use biei::config::{BlCapacityPolicy, Tier1Strategy};
...
fn config_snapshot(config: &SimConfig) -> Value {
    ...
    let biei::config::GossipConfig { publish_interval } = gossip;
    let biei::config::RoutingConfig {
        tier1_strategy,
        tier3_enabled,
        drain_max_queue,
    } = routing;
...
fn range_snapshot(range: biei::config::CostRange) -> Value {
