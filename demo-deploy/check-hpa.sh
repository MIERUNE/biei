#!/usr/bin/env bash
# Regression check for the portable, CPU-only autoscaling example.
set -euo pipefail
...
# Preserve enough idle CPU for a render and permit fast burst expansion.
expect "$metrics" 'averageUtilization: 50' 'CPU target must retain 50% headroom'
...
# Scale-down intentionally protects process-local renderer/resource warmth.
# Keep this explicit so making it faster is a reviewed policy change rather
# than an accidental side effect of editing the HPA.
expect "$scale_down" 'stabilizationWindowSeconds: 300' 'scale-down must retain its 5-minute stabilization window'
expect_policy "$scale_down" scale-down Pods 1 60

printf 'PASS: portable CPU HPA scale-up and scale-down policies are explicit\n'
